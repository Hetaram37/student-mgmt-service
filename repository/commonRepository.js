...
exports.findById = async (id, projection, collection) => {
  console.debug('Find one data: id: %s, projection: %j, collection: ', id, projection, collection)
  try {
    const doc = await collection.findById(id, projection).lean()
    return doc
  } catch (error) {
    console.error(`Error while getting data from ${collection}: %s %j`, error, error)
    throw error
...
exports.findOne = async (id, projection, collection) => {
  console.debug('Find one data: id: %s, projection: %j, collection: ', id, projection, collection)
  try {
    const doc = await collection.findOne(id, projection).lean()
    return doc
  } catch (error) {
    console.error(`Error while getting data from ${collection}: %s %j`, error, error)
    throw error
